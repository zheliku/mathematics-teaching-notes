...
#example(
  ...
  choices: choices22(
    row-gutter: 2em,
    [$display((-infinity, 5/4])$],
    [$display((1, 5/4])$],
    [$display([-3/2, +infinity))$],
    [$display((-infinity, 2])$],
  ),
  answer: [#tab *B*],
)
...
#example(
  question: [
    【2021江苏无锡大桥中学高一期中】若函数 $f(x) = display(x/(x + 1))$，则 $display(f(1) + f(2) + ... + f(50) + f(1/2) + f(1/3) + ... + f(1/50)) =$ #blank()。
  ],
  answer: [#tab *49.5*],
)
...
#example(
  question: [
    【2023-2024安徽省部分学校高一上期末】已知函数 $f(x) = display(2/(1 + 3^x))$，则 $f(-2024) + ... + f(-1) + f(0) + f(1) + ... + f(2024)$ 的值为（#h(3em)）
  ],
  choices: choices14(
    [$4047$],
    [$4048$],
...
#note[
  *二次对称产周期*
  1. 若 $f(x)$ 有两条对称轴 $x = a, x = b$，则 $T = 2 abs(b - a)$。
  2. 若 $f(x)$ 有两个对称中心 $(a, c), (b, c)$，则 $T = 2 abs(b - a)$。
  3. 若 $f(x)$ 有一条对称轴 $x = a$ 和一个对称中心 $(b, c)$，则 $T = 4 abs(b - a)$。
]

#example(
  question: [
    【广东省广州市三校2023-2024高一上期末联考】已知奇函数 $f(x)$ 的图象关于直线 $x = 1$ 对称，当 $x in [0, 1]$ 时，$f(x) = 2^x + b$，则 $display(f(2023/2))$ 的值为（#h(3em)）
  ],
  choices: choices22(
    [$-1 - sqrt(2)$],
    [$1 - sqrt(2)$],
...
#example(
  question: [
    【广东省佛山市2023-2024高一上期末】(多选) 已知函数 $f(x)$ 满足：对任意的 $x in R$，都存在 $f(-x) = -f(x)$，$display(f(1/2 - x) = f(3/2 + x))$，且 $f(1) = 2$，则（#h(3em)）
  ],
  choices: choices22(
    row-gutter: 1.5em,
    [$y = f(x)$ 关于 $(1, 0)$ 对称],
...
#think[
  ...
  + $display(f(x) = display((a x + b)/(c x + d)))$ 的图像由哪个基础函数变换得到？
]
...
#example(
  question: [
    【2019海淀二模】把函数 $y = 2^x$ 的图象向右平移 t 个单位长度，所得图象对应的函数解析式为 $y = display((2^x)/3)$，则 $a$ 的值为（#h(3em)）
  ],
  choices: choices14(
    [$display(1/2)$],
    [$log_2 3$],
...
#example(
  question: [
    【重庆市第八中学校2023-2024高一上期末】设函数 $f(x)$ 的定义域为 $bb(R)$，$f(x + 1)$ 为奇函数，$f(x + 2)$ 为偶函数，当 $x in [1, 2]$ 时，$f(x) = a x^2 + b$。若 $f(3) + f(4) = 6$，则 $f(display(13/3))$ 的值为（#h(3em)）
  ],
  choices: choices14(
    [$display(-4/3)$],
    [$display(32/9)$],
...
#example(
  question: [
    【湖南长沙市长郡中学2023-2024高一上期末】(多选) 定义在 $bb(R)$ 上的函数 $f(x)$ 满足 $f(4 - x) = -f(x)$，$f(2 x + 1)$ 为偶函数，$f(1) = 2$。函数 $g(x) (x in bb(R))$ 满足 $g(x) = g(2 - x)$，若 $y = f(x)$ 与 $y = g(x)$ 恰有 2023 个交点，从左至右依次为 $(x_1, y_1), (x_2, y_2), ..., (x_2023, y_2023)$，则下列说法正确的是（#h(3em)）
  ],
  choices: choices22(
    [$f(x)$ 为奇函数],
    [$2$ 为 $y = f(x)$ 的一个周期],
...
#example(
  question: [
    【2022全国新高考II】已知 $f(x)$ 的定义域为 $bb(R)$，且 $f(x + y) + f(x - y) = f(x) f(y), f(1) = 1$，则 $display(sum_(k=1)^(22) f(k)) =$ （#h(3em)）
  ],
  choices: choices14(
    [$-3$],
    [$-2$],
...
#think[
  1. 满足 $f(x + y) + f(x - y) = 2 f(x) f(y) (forall x, y in bb(R))$ 的 $f(x)$ 是偶函数吗？
]
