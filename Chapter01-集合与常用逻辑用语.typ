...
#definition[（集合）][
  #tab 我们把研究对象统称为*元素*， 一些元素组成的总体叫做*集合*。
]
...
== 空集
#definition[（空集）][
  #tab 不包含任何元素的集合称为*空集*。记作 $emptyset$ 或 ${}$.
]
...
#think[
  #tab $emptyset、0、{0}、{emptyset}$ 之间的关系？
]

== 有限集合的子集个数
#align(center)[
  #table(
    columns: (2fr, 5fr, 1.5fr, 1.5fr, 1.5fr),
    stroke: none,
    align: horizon,
    inset: 0.5em,
    table.hline(),
...
== 区间
#definition[（区间）][
  #tab 数轴某一段上所有点对应的所有#underline[连续实数]组成的集合，称为*区间*。
]
...
#definition[（命题）][
  #tab 我们把可以判断真假的陈述句，叫做*命题*。其中判断为真的陈述句叫做*真命题*，判断为假的陈述句叫做*假命题*。
]
